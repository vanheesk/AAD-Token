...
using GetTokenFromAzureEntra.Commands.Get;
...
public class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        {
            config.SetApplicationName("aad-token");
            config.ValidateExamples();
            //config.AddExample("get", "--TenantId <TENANT-ID>");

            // Get
            config.AddBranch<GetSettings>("get", add =>
            {
                add.SetDescription("Get an access token from Azure AD");
                add.AddCommand<GetTokenCommand>("token");
            });
        });
